...
// this doesn't produce a pmulhrsw
// but, it does get turned into a pmaddwd after inlining, so that's something
#[inline(always)]
fn multiply_high(a: i16x8, b: i16x8) -> i16x8 {
    ((a.cast::<i32>() * b.cast::<i32>()) >> i32x8::splat(15)).cast::<i16>()
}

fn saturating_narrow(a: i16x8) -> u8x8 {
    a.simd_clamp(i16x8::splat(0), i16x8::splat(255)).cast::<u8>()
}
...
pub unsafe fn dequantize_and_idct_block_8x8(
    ...
    for i in 0..8 {
        ...
        std::ptr::write::<u8x8>(
            ...
            saturating_narrow(i16x8::shr(data_with_offset, i16x8::splat(SHIFT + 3)))
        );
...
pub unsafe fn color_convert_line_ycbcr(y: &[u8], cb: &[u8], cr: &[u8], output: &mut [u8]) -> usize {
    ...
    for i in 0..num_vecs {
        const SHIFT: i16 = 6;
        // Load.
        let y: u8x8 = std::ptr::read(y.as_ptr().wrapping_add(i * 8) as *const _);
        let cb: u8x8 = std::ptr::read(cb.as_ptr().wrapping_add(i * 8) as *const _);
        let cr: u8x8 = std::ptr::read(cr.as_ptr().wrapping_add(i * 8) as *const _);


        // Convert to 16 bit.
        let y = y.cast::<i16>().shl(i16x8::splat(SHIFT));
        let cb = cb.cast::<i16>().shl(i16x8::splat(SHIFT));
        let cr = cr.cast::<i16>().shl(i16x8::splat(SHIFT));
        ...
        let r = saturating_narrow(i16x8::shr(r, i16x8::splat(SHIFT)));
        let g = saturating_narrow(i16x8::shr(g, i16x8::splat(SHIFT)));
        let b = saturating_narrow(i16x8::shr(b, i16x8::splat(SHIFT)));
